...
If you ever decide to edit a mod that has already been served to users, as in they joined the server, and don't increment the version of the mod you edited, then clients will encounter an error when trying to join that event once the updated mod version is live. To avoid this error you need to increment the mod version when you are doing the update, or simply create a whole new mod.

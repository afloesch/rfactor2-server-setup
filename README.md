...
> Note: the connection type dropdown simply has a few preset values for the upload and download speeds of some old and outdated home internet connection types. These options were probably relevant in 2012, but almost nobody will be using an ISDN or DSL line anymore. The Upload and Download speeds basically tell the server how much throughput the network has available on the WAN, and so the server will then adjust the number of participants it should be able to host based on what is specified for the connection speed. If you are following the AWS portion of the guide you do not need to change the Upload or Download speed values, what is shown is what the server has. For an experienced admin this is actually a great feature, but for the average person probably just confusing.
